...
const MobileBottomSheet: React.FC<MobileBottomSheetProps> = ({ 
  ...
  // Combined categories (vendors + infrastructure)
  const allCategories = [
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    if (showDetailView) return; // Disable dragging in detail view
    setIsDragging(true);
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (!isDragging || showDetailView) return; // Disable dragging in detail view
    setCurrentY(e.touches[0].clientY);
  };
